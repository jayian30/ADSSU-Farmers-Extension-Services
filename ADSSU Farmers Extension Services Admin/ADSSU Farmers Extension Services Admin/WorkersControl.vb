...
Public Class WorkersControl
    ...
    Public Sub New()
        ...
        btnDelete = New Button() With {.Text = "Delete Worker", .Width = 120, .Height = 30, .Top = 10, .Left = 330}
        ...
        AddHandler btnAdd.Click, AddressOf AddWorker
        AddHandler btnEdit.Click, AddressOf EditWorker
        AddHandler btnRefresh.Click, AddressOf LoadData
        AddHandler btnDelete.Click, AddressOf DeleteWorker
        ...
        Try
            Dim dtBrgy = db.ExecuteQuery("SELECT DISTINCT assigned_barangay FROM extension_workers WHERE assigned_barangay IS NOT NULL AND assigned_barangay != '' ORDER BY assigned_barangay")
            For Each row As DataRow In dtBrgy.Rows
                cmbBarangay.Items.Add(row("assigned_barangay").ToString())
            Next
    ...
    Public Sub LoadData()
        Try
            Dim query As String = "SELECT w.id, u.username, u.full_name, w.contact_number, w.assigned_barangay, w.created_at FROM extension_workers w JOIN users u ON w.user_id = u.id WHERE 1=1"
            Dim parameters As New Dictionary(Of String, Object)
            
            If cmbBarangay IsNot Nothing AndAlso cmbBarangay.SelectedIndex > 0 Then
                query &= " AND w.assigned_barangay = @brgy"
    ...
    Private Sub AddWorker()
        Dim dlg As New WorkerDialog()
        If dlg.ShowDialog() = DialogResult.OK Then
            LoadData()
        End If
    End Sub

    Private Sub EditWorker()
    ...
    Private Sub DeleteWorker()
        If dgv.SelectedRows.Count > 0 Then
            Dim id = dgv.SelectedRows(0).Cells("id").Value.ToString()
            Dim name = dgv.SelectedRows(0).Cells("full_name").Value.ToString()
            If MessageBox.Show("Are you sure you want to delete worker '" & name & "'?", "Confirm", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Try
                    db.ExecuteNonQuery("DELETE FROM extension_workers WHERE id = @id", New Dictionary(Of String, Object) From {{"@id", id}})
                    LoadData()
                Catch ex As Exception
    ...
    Private Sub PrintData()
        Dim printer As New UIHelper.GridPrinter(dgv, "Extension Workers Report")
        printer.Print()
    End Sub

    Private Sub ExportData()
        UIHelper.ExportGridToExcel(dgv, "ExtensionWorkers.xls")
    End Sub
End Class
